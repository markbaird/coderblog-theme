...
module.exports = function IndexModule(pb) {
  ...
  var BlogArticleService = pb.PluginService.getService('blog_article_service', 'coderblog-theme');
  ...
  Index.prototype.render = function(cb) {
    ...
    contentService.getSettings(function(err, contentSettings) {
      self.gatherData(pageNumber, contentSettings.articles_per_page, function(err, data) {
        // Check that data.content isn't empty
        if (!data.content || data.content.length == 0) {
          self.getEmptyTemplate(data.content, function (err, template) {
            if (util.isError(err)) {
              throw err;
            }
        ...
        ArticleService.getMetaInfo(data.content[0], function(metaKeywords, metaDescription, metaTitle, metaThumbnail) {
          ...
          if (data.content.length >= contentSettings.articles_per_page) {
            // Would be better if we could do a check here to see if there really is a next page
  ...
  Index.prototype.getEmptyTemplate = function(content, cb) {
    cb(null, 'empty');
    return;
  };
  ...
  Index.prototype.gatherData = function(pageNumber, perPage, cb) {
    var self  = this;
    var tasks = {
      ...
      content: function(callback) {
        self.loadContent(pageNumber, perPage, callback);
      },
  ...
  Index.prototype.loadContent = function(pageNumber, perPage, articleCallback) {
    ...
    var service = new BlogArticleService();
    if(this.req.pencilblue_preview) {
      ...
      service.find({ article_topics: topic }, opts, articleCallback);
    }
    else if(article) {
  ...
  Index.prototype.getContentSpecificPageName = function(content, cb) {
    ...
    if(this.req.pencilblue_article || this.req.pencilblue_page) {
      ...
      var objId = this.req.pencilblue_section || this.req.pencilblue_topic;
      var dao     = new pb.DAO();
      dao.loadById(objId, objType, function(err, obj) {
